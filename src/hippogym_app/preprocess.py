...
def find_distances(minutiae_list, feature_norm):
    ...
    for m1_idx, m2_idx in combinations(range(mn), 2):
        distances[m1_idx, m2_idx] = distances[m2_idx, m1_idx] = euclidian_distance(minutiae_list[m1_idx], minutiae_list[m2_idx])
...
def expand_minutia(minutia_idx, minutiae, distances, argsorted_distances, factor, feature_norm):
    ...
    neighbors_number = np.searchsorted(distances[argsorted_distances], [factor, factor*2, factor*3, factor*4], side='right')-1
    ...
    for neighbor_idx in np.concatenate((argsorted_distances[1:4], argsorted_distances[-3:])):
    ...
    return list(near_dist) + list(far_dist) + list(neighbors_number/(len(minutiae)-1 if feature_norm else 1)) + angles + [discretize(minutia[3])]

def get_one_pair_regresor_data(xml_path, FEATURE_NORM=True):
    latent_xml = xml_path
    latent_minutiae = get_minutiae_from_xml(latent_xml)
    
    latent_minutiae_distances, latent_distances_factor = find_distances(latent_minutiae, FEATURE_NORM) 
    ...
    for latent_minutia_idx in range(len(latent_minutiae)):
        argumented_latent_minutia = expand_minutia(  latent_minutia_idx, latent_minutiae, 
                                                    latent_minutiae_distances[latent_minutia_idx], 
                                                    argsorted_latent_minutiae_distances[latent_minutia_idx], 
                                                    latent_distances_factor, FEATURE_NORM)
