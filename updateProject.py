...
def upload_step_files(steps, projectConfig):
    ...
    for file in steps:
        if file and file != "game":
            upload_file(
                file,
                projectConfig.get("awsSetup").get("bucket"),
                projectConfig.get("id"),
            )
...
def check_task_definition(projectConfig):
    ...
    client = boto3.client(
        "ecs", region_name=projectConfig.get("awsSetup").get("region")
    )
...
def check_repository(projectConfig):
    ...
    client = boto3.client(
        "ecr", region_name=projectConfig.get("awsSetup").get("region")
    )
    ...
    for repo in repositories:
        if repo.get("repositoryName") == projectConfig.get("awsSetup").get(
            "repository"
        ):
...
def create_repository(projectConfig):
    ...
    projectConfig["awsSetup"]["repositoryUri"] = response.get("repository").get(
        "repositoryUri"
    )
    logging.debug(response)
...
def check_image(projectConfig):
    ...
    response = client.list_images(
        repositoryName=projectConfig.get("awsSetup").get("repository"), maxResults=1
    )
    logging.debug(response)
...
def push_image(projectConfig, imageExists):
    ...
    with open(xvfb_path, "r") as infile:
        ...
        if "App dev" in instructions:
            confirm = input(
                f"dev flag set in {xvfb_path} file, ssl and s3uploading disabled."
    ...
    output = os.system(
        f"aws ecr get-login-password --region ca-central-1 | docker login --username AWS --password-stdin {repoAddress}"
    )
    logging.info(output)
